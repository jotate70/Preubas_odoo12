...
class GeneralLedgerReportCompute(models.TransientModel):
    ...
    def compute_data_for_report(self,
                                ...
                                with_partners=True, partners_details_all=False):
        self.ensure_one()
        # Compute report data
        ...
        if with_partners:
            ...
            if partners_details_all: # SI CUMPLE ESTA VALIDACION ENTRARÁ A MÉTODO.
                self._inject_partner_values_add_details()
            else:
...
AND
    ...
    def _inject_partner_values_add_details(self, only_empty_partner=False):
        """ Inject report values for report_general_ledger_partner.
            ...
            INNER JOIN
                account_analytic_tag aat
                    ON
                        ...
                        AND aat.id IN %s
            """
       #APLICAR PARA TODAS LAS CUENTAS CONTABLES RELACIONADAS AL PARTNER
        query_inject_partner += """
            WHERE
                ra.report_id = %s
            AND
                ra.is_partner_account in (TRUE,FALSE)
        """
        if not only_empty_partner:
            query_inject_partner += """
            AND
